import { client, createDatabaseConnection, checkDatabaseHealth } from './connection';
...
export async function getDatabaseInfo(): Promise<DatabaseInfo> {
  ...
  if (isHealthy) {
    const { client: infoClient } = createDatabaseConnection(config.database.url, {
      maxConnections: 1,
      prepare: false,
    });
    ...
    try {
      ...
      console.warn('Warning: Could not fetch database statistics:', getErrorMessage(error));
    } finally {
...
export async function testDatabaseConnection(retries: number = 3): Promise<boolean> {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      console.warn(`Database connection attempt ${attempt}/${maxRetries} failed:`, getErrorMessage(error));
    }
 ...
 */
export async function waitForDatabase(timeoutMs: number = 30000): Promise<boolean> {
...
export async function validateDatabaseSchema(): Promise<{
  ...
  try {
    ...
    const { client: validationClient } = createDatabaseConnection(config.database.url, {
      maxConnections: 1,
      prepare: false,
    });
    ...
    try {
      ...
      const migrationTable = tableNames.find(name => name === 'drizzle_migrations');
      if (migrationTable) {
        const migrationCount = await validationClient`
          SELECT COUNT(*) as count FROM drizzle_migrations
        `;
        
        if (parseInt(migrationCount[0].count) > 0 && tableNames.length === 1) {
          result.errors.push('Migrations have been run but no application tables found');
          result.isValid = false;
        }
      }
    } finally {
...
export async function getDatabaseStats(): Promise<{
  ...
  try {
    ...
    const totalRows = rowCounts.reduce((sum, table) => sum + (parseInt(table.row_count) || 0), 0);
    ...
    return {
      ...
      connectionCount: parseInt(connectionResult[0].count),
    };
