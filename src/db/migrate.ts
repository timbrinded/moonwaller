...
import { db, client, createDatabaseConnection, checkDatabaseHealth } from './connection';
...
export async function runMigrations(targetEnvironment?: string) {
  ...
  if (!isHealthy) {
    console.error('❌ Database health check failed. Please ensure the database is running.');
    process.exit(1);
  }
  ...
  try {
    ...
    `;
    console.log(`📊 Database now contains ${migrationCheck[0].count} tables`);
    ...
    if (error.message.includes('ECONNREFUSED')) {
      ...
      console.error('💡 Hint: Database does not exist. Check your DATABASE_URL and ensure the database is created.');
    }
    ...
    try {
      ...
      console.warn('Warning during migration cleanup:', error.message);
    }
  }
}
...
export async function rollbackMigrations(steps: number = 1) {
  ...
  console.log(`🔄 Rolling back ${steps} migration(s) for ${config.env} environment...`);
  ...
  try {
    ...
    console.log('💡 For now, use "bun run db:reset" followed by "bun run db:migrate"');
  } catch (error) {
