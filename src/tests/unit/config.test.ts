...
describe('Database Configuration', () => {
  ...
  it('should prefer Neon URL in production', async () => {
    ...
    process.env.NEON_DATABASE_URL = 'postgresql://neon-user:pass@neon.tech:5432/prod';
    ...
    expect(config.database.url).toBe('postgresql://neon-user:pass@neon.tech:5432/prod');
  ...
  it('should fallback to Supabase URL if Neon not available', async () => {
    ...
    process.env.SUPABASE_DATABASE_URL = 'postgresql://supabase:pass@supabase.co:5432/prod';
    process.env.DATABASE_URL = 'postgresql://regular:pass@localhost:5432/prod';
    process.env.JWT_SECRET = 'production-jwt-secret';
    process.env.API_KEY = 'production-api-key';
    ...
    expect(config.database.url).toBe('postgresql://supabase:pass@supabase.co:5432/prod');
  });

  it('should use custom environment variables when provided', async () => {
    process.env.NODE_ENV = 'development';
    process.env.DATABASE_URL = 'postgresql://custom:pass@custom.host:5432/custom_db';
    
    // Re-import config to get fresh instance
    delete require.cache[require.resolve('../../shared/config')];
    const { config } = await import('../../shared/config');
    
    expect(config.database.url).toBe('postgresql://custom:pass@custom.host:5432/custom_db');
  });
});